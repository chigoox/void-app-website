...
export const MenuButton = ({ item, onPage, setOnPage }) => {
    ...
    const currentPage = url == item

    return (
        < button onClick={onClick} className={`${(currentPage || url != '') ? ' h-32 md:z-50 ' : 'h-[9.5rem] md:h-full'} relative center trans-slow  text-black :hover-border-2 group md:w-96 bg-red-400  md:overflow-hiddenm`} >
            ...
            <h1 className={`${currentPage ? 'text-xl text-red-400 bg-opacity-50 w-28' : 'text-3xl w-fit text-red-500'} bg-black bg-opacity-70 p-2 font-bold text-center  absolute z-10 trans-slow hover:scale-110 rounded`}>{item}</h1>
            <img className='object-cover h-full w-full group-hover:scale-105 trans-slow' src={
                (item == 'CodingClass') ? 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2340&q=80' :
